...
const getAll = async (req, res) => {
  try {
    ...
    if (search) {
      // Regex search (works without text index, case-insensitive)
      query.$or = [
...
const create = async (req, res) => {
  try {
    ...
    const product = await Product.create({ name, description, price: Number(price), image, category, stock: Number(stock) || 100 });
